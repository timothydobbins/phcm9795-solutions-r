...
\tightlist
\item
  Module 10: Corrected Activity 10.3 (d)
\end{itemize}
...
\begin{quote}
As in Question (1), we need to specify r. Note that for \texttt{epi.sscc()}, r represents ``the number in the control group divided by the number in the case group''. \emph{This is in contrast to r for \texttt{epi.sscohortc()} and \texttt{epi.sscompc()}.} So 3 controls per case gives a ratio of 3:
\end{quote}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{epi.sscc}\NormalTok{(}\AttributeTok{OR=}\FloatTok{1.5}\NormalTok{, }\AttributeTok{p0=}\FloatTok{0.2}\NormalTok{, }\AttributeTok{n=}\ConstantTok{NA}\NormalTok{, }\AttributeTok{power=}\FloatTok{0.9}\NormalTok{, }\AttributeTok{r=}\DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
